...
void water_init(void) // khoi tao qua trinh lay mau nuoc
{
	...
	motor_setup_speed(MOTORZ, HIGH_SPEED);
	...
	water.flag_motor = 0;
	O_CTRL1(0); // off van hut
	O_CTRL2(0); // off van thoi
	O_CTRL3(0); // off PUMP hut/thoi
}
...
{
	O_CTRL1(1); // on van hut
	O_CTRL2(1); // off van thoi
	O_CTRL3(0); // on bom hut
...
void water_balance(void)
{
	...
	O_CTRL1(1); // off van hut 
	O_CTRL2(0); // on van thoi
	O_CTRL3(1); // on bom hut
...
void loop_water(uint8_t input) // check loop process sample water
{
	...
	if(!tick_checkMS(&water.tick_wait_off_motor, 4000) && water.flag_balance)
	{
		motor_fast_stop(MOTORZ);
		tick_initTime(&water.tick_trigger);
	}

	if(!tick_checkMS(&water.tick_discharge, 3000) && water.flag_discharge)
	{
		INFO("Wait discharge water \n");
		water.flag_discharge = 1;
		//water.flag_complete = 1;
	}

	sensor_manager();

	if(sensor_get(SS_MT3) > sensor_get(SS_LM3))
	{
