...
void KeyGet(uint8_t port_id)
{
	...
	switch(port_id)
	{
		case 0: 
			keyGet = key1_in;
			if(keyGet == !key1) key.ext[port_id].press = KEY_PRESS;
			else key.ext[port_id].press = KEY_RELEASE;
			break;
		case 1: 
			keyGet = key2_in;
			if(keyGet == !key2) key.ext[port_id].press = KEY_PRESS;
			else key.ext[port_id].press = KEY_RELEASE;
			break;
		case 2: 
			keyGet = key3_in;
			if(keyGet == !key3) key.ext[port_id].press = KEY_PRESS;
			else key.ext[port_id].press = KEY_RELEASE;
			break;
		case 3: 
			keyGet = key4_in;
			if(keyGet == !key4) key.ext[port_id].press = KEY_PRESS;
			else key.ext[port_id].press = KEY_RELEASE;
			break;
        default:
            break;
